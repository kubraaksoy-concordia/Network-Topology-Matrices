...
lemma cutset_vec_index_one_alt: 
  ...
  by (metis (no_types, lifting) neg_cut_not_pos neg_cutset_altdef nth_mem wf_invalid_edge)
...
lemma cutset_rvec_index_mone: "j < length Es  \<Longrightarrow> cs = cutset_edges u \<Longrightarrow> (Es!j) \<notin> set (pos_cut_list u) \<Longrightarrow> cutset_rvec u cs Es $ j = -1  \<longleftrightarrow> (Es!j) \<in> set (neg_cut_list u) \<and> (Es ! j) \<in> set cs"
  ...
  by (metis (no_types, lifting) invalid_cut_pos_neg  wf_invalid_cs zero_neq_neg_one)
...
proof -
  ...
  then show ?thesis
    using a5 a4 a3 a2
    by (metis cutset_rvec_index nonempty_network_system.pos_cut_list_alt one_neq_zero pos_cutset_altdef wf_nonempty_netw_list_sys)
qed
