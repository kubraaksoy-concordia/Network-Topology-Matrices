...
locale cutset_system = nonempty_network_system +
  ...
  assumes wf_2: "U \<in> set N \<Longrightarrow> (cutset U) \<in> set \<C>s" 
begin 
...
shows "cutset_system \<N>s \<E>s \<C>s \<longrightarrow> (cutset U) \<in> set \<C>s"
  using wf_2 assms by auto

lemma wf_cutset_system_iff: 
  assumes "cs \<in> \<C>" and "distinct \<C>s" and "U \<in> set N"
  shows "cutset_system \<N>s \<E>s \<C>s \<longleftrightarrow>  set cs \<subset> \<E> \<and>  (cutset U) \<in> set \<C>s"
...
lemma wf_invalid_cs:
  shows  "cutset U \<notin> set \<C>s \<Longrightarrow> U \<notin> set N" 
  using wf_2 by auto
...
lemma wf_cutsets_sub : "\<forall>t \<in> {0..<length N}. (cutset (N!t)) \<in> \<C>"
...
definition "C\<^sub>p \<equiv> {(u, e, cs). cs \<in> set \<C>s \<and> e \<in> set cs \<and>  e \<in> set (pos_cut_list u) \<and>  e \<in> set (cutset u)}"   (*positively directed cutset*)

definition "C\<^sub>n \<equiv> {(u, e, cs). cs \<in> set \<C>s \<and> e \<in> set cs \<and> e \<in> set (neg_cut_list u) \<and> e \<in> set (cutset u)}" (*negatively directed cutset*)
                                                                                                         
definition in_pos_cutset :: "'a nodes \<Rightarrow> 'a edge \<Rightarrow> 'a edges \<Rightarrow> bool"  where         
"in_pos_cutset u e cs \<equiv> (u, e, cs) \<in> C\<^sub>p"

definition in_neg_cutset :: "'a nodes \<Rightarrow>'a edge \<Rightarrow> 'a edges \<Rightarrow> bool"  where
"in_neg_cutset u e cs \<equiv> (u, e, cs) \<in> C\<^sub>n"
         
lemma pos_cutset_altdef: "in_pos_cutset u e cs \<longleftrightarrow> e \<in> set \<E>s \<and> cs \<in> set \<C>s \<and> e \<in> set cs \<and> u \<in> set N \<and> e \<in> set (pos_cut_list u) \<and> e \<in> set (cutset u)"
  using C\<^sub>p_def in_pos_cutset_def  pos_cut_list_alt by auto         

lemma in_pos_cutset: assumes "cs \<in> set \<C>s" assumes "e \<in> set cs"
  shows "in_pos_cutset u e cs \<longleftrightarrow>  e \<in> set (pos_cut_list u) \<and> e \<in> set (cutset u)"
  using assms pos_cut_list_alt pos_cutset_altdef by blast

lemma pos_cutsetI:  "cs \<in> set \<C>s \<Longrightarrow> e \<in> set cs \<Longrightarrow>  e \<in> set (pos_cut_list u) \<Longrightarrow>  e \<in> set (cutset u) \<Longrightarrow> in_pos_cutset u e cs"
  using C\<^sub>p_def by (simp add: pos_cut_list_alt pos_cutset_altdef)   

 lemma pos_cutsetE:        
  shows "in_pos_cutset u e cs \<Longrightarrow> cs \<in> set \<C>s \<Longrightarrow> e \<in> set cs \<Longrightarrow>  e \<in> set (pos_cut_list u) \<Longrightarrow>  e \<in> set (cutset u)"
   using in_pos_cutset_def pos_cutset_altdef by blast    
                                                   
lemma neg_cutset_altdef: "in_neg_cutset u e cs \<longleftrightarrow>  cs \<in> set \<C>s \<and> e \<in> set cs \<and> e \<in> set (neg_cut_list u) \<and> e \<in> set (cutset u)"
      by (simp add:C\<^sub>n_def in_neg_cutset_def)

lemma in_neg_cutset: assumes "e \<in> set \<E>s" assumes "cs \<in> set \<C>s" assumes "e \<in> set cs"
  shows "in_neg_cutset u e cs \<longleftrightarrow>  e \<in> set (neg_cut_list u) \<and> e \<in> set (cutset u)"
  using assms(2) assms(3) neg_cut_list_alt neg_cutset_altdef by blast

lemma neg_cutsetI: "e \<in> set \<E>s \<Longrightarrow> cs \<in> set \<C>s \<Longrightarrow>  e \<in> set cs \<Longrightarrow> u \<in> set N \<Longrightarrow> e \<in> set (neg_cut_list u) \<and> e \<in> set (cutset u) \<Longrightarrow> in_neg_cutset u e cs"
...
lemma not_pos_not_neg_notin_cutsets: "e \<notin> set (cutset u) \<Longrightarrow> \<not> in_pos_cutset u e cs \<Longrightarrow> \<not>in_neg_cutset u e cs"
...
\<Longrightarrow> \<not> (in_pos_cutset (N!i) (\<E>s!j) (\<C>s!i)) \<Longrightarrow> \<not> in_neg_cutset (N!i) (\<E>s!j) (\<C>s!i) \<Longrightarrow> (\<E>s!j) \<notin> set (\<C>s!i)"
   using   neg_cutset_cond_indexed pos_cutset_cond_indexed cut_list_pos_neg cutset_def 
   by (smt (verit, best) mem_Collect_eq neg_cutset_altdef pos_cutsetI set_filter valid_cutset_index)                                                           
